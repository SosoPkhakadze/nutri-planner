        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              {/* Icon in center */}
              <div className={`absolute inset-0 flex items-center justify-center w-16 h-16 rounded-full ${colorClass}`}>
                {icon}
              </div>
            </div>
...
const QuickActionButton = ({ icon, label, onClick, primary = false }: any) => (
  <button
    onClick={onClick}
...
export default function DashboardClientPage({ 
  ...
  // Calculate completion percentages for visual feedback
        ...
        <main className="container mx-auto p-4 md:p-8 space-y-8">
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                <div className="flex items-center gap-6">
                  <div className="flex items-center bg-slate-800/50 rounded-xl p-1">
                    <Link href={`/?date=${date.prevDay}`} className="p-3 rounded-lg hover:bg-slate-700 transition-colors">
                    ...
                    <div className="px-6">
                      <h1 className="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent">
                        {date.displayDateLocale}
                      </h1>
                      <p className="text-slate-400 text-sm mt-1">
                        {completedMeals} of {totalMeals} meals completed
                      </p>
                    </div>
                    <Link href={`/?date=${date.nextDay}`} className="p-3 rounded-lg hover:bg-slate-700 transition-colors">
              ...
              {totalMeals > 0 && (
                <div className="mt-6">
                  <div className="flex items-center justify-between text-sm text-slate-400 mb-2">
                    ...
                    <span>{Math.round(mealCompletionRate)}% Complete</span>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard 
              icon={<Flame size={24} />} 
              label="Calories" 
              value={Math.round(dailyTotals.consumedCalories)} 
            ...
            <StatCard 
              ...
              label="Protein" 
              value={Math.round(dailyTotals.consumedProtein)} 
            ...
            <StatCard 
              icon={<Zap size={24} />} 
              label="Carbs" 
              value={Math.round(dailyTotals.consumedCarbs)} 
            ...
            <StatCard 
              icon={<Apple size={24} />} 
              label="Fat" 
              value={Math.round(dailyTotals.consumedFat)} 
              target={targets.fat} 
              unit="g" 
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
            {/* Meals Section - Takes up more space */}
                  ...
                  {meals.length > 0 ? (
                    <SortableContext items={meals.map(m => m.id)} strategy={verticalListSortingStrategy}>
                      <div className='space-y-6'>
                        {meals.map((meal) => (
                          <DraggableMealCard key={meal.id} meal={meal} foodItems={foodItems} />
                        ))}
                      </div>
                    </SortableContext>
                  ) : (
                    <div className="text-center py-16">
                      <div className="mb-6">
                        ...
                        <p className="text-slate-400 max-w-sm mx-auto">
                          Start building your nutrition plan by adding your first meal of the day.
                        </p>
                      </div>
