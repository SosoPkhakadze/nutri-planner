...
import { ChevronLeft, ChevronRight, Flame, Zap, Apple, Drumstick, Plus, Calendar, Target, Copy } from 'lucide-react';
...
// FIX: Import reorderMealFoods alongside reorderMeals
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <svg className="w-16 h-16 transform -rotate-90" viewBox="0 0 80 80">
                ...
                <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="4" fill="none" strokeDasharray={strokeDasharray} strokeDashoffset={strokeDashoffset} className={`${colorClass.split(' ')[1]} transition-all duration-700 ease-out`} strokeLinecap="round" />
              </svg>
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{value.toLocaleString()}<span className="text-base text-slate-400 font-normal ml-1">{unit}</span></p>
              <div className="flex items-center gap-2 mt-1">
                <div className="w-12 h-1.5 bg-slate-700 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-700 ease-out ${colorClass.split(' ')[0]}`} style={{ width: `${Math.min(percentage, 100)}%` }} /></div>
...
const QuickActionButton = ({ icon, label, primary = false }: any) => (
  <button className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 ${primary ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white hover:from-cyan-500 hover:to-blue-500 shadow-lg shadow-cyan-500/25' : 'bg-slate-800/50 text-slate-300 hover:bg-slate-700/50 border border-slate-700/50'} hover:scale-105 hover:shadow-lg`}>
    {icon}
    <span>{label}</span>
  </button>
);
...
export default function DashboardClientPage({ 
  ...
  const generateTitle = () => {
    ...
    const displayDate = new Date(`${date.displayDateString}T00:00:00.000Z`);
    ...
    if (date.displayDateString === localTodayString) {
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                <div className="flex items-center gap-6">
                  <div className="flex items-center bg-slate-800/50 rounded-xl p-1">
                    ...
                    <div className="px-6">
                      <h1 className="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent">{pageTitle}</h1>
                ...
                <div className="flex items-center gap-3">
                  <CopyDayModal sourceDate={date.displayDateString} hasMeals={hasMeals}><button disabled={!hasMeals} className="px-4 py-2 bg-slate-600 hover:bg-slate-700 text-white font-semibold rounded-md transition flex items-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"><Copy size={16} />Copy Day</button></CopyDayModal>
                  <SaveDayAsTemplateModal date={date.displayDateString} hasMeals={hasMeals} />
                </div>
              </div>
              {totalMeals > 0 && (<div className="mt-6"><div className="flex items-center justify-between text-sm text-slate-400 mb-2"><span>Daily Progress</span><span>{Math.round(mealCompletionRate)}% Complete</span></div><div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full transition-all duration-700 ease-out" style={{ width: `${mealCompletionRate}%` }} /></div></div>)}
            </div>
          </div>
              ...
              <Card className="overflow-hidden bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-sm border-slate-700/50">
                <div className="p-6 border-b border-slate-700/50 bg-gradient-to-r from-slate-800/50 to-transparent">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex items-center gap-3"><div className="p-2 bg-cyan-500/20 rounded-lg"><Target size={24} className="text-cyan-400" /></div><div><h2 className="text-2xl font-bold text-white">Today's Meals</h2><p className="text-slate-400 text-sm">Plan and track your nutrition</p></div></div>
                    <AddMealModal date={date.displayDateString}><QuickActionButton icon={<Plus size={20} />} label="Add Meal" primary={true} /></AddMealModal>
                  </div>
                </div>
                <div className="p-6">
                  {meals.length > 0 ? (<SortableContext items={meals.map(m => m.id)} strategy={verticalListSortingStrategy}><div className='space-y-6'>{meals.map((meal) => (<DraggableMealCard key={meal.id} meal={meal} foodItems={foodItems} />))}</div></SortableContext>) : (<div className="text-center py-16"><div className="mb-6"><div className="w-16 h-16 mx-auto bg-slate-800 rounded-full flex items-center justify-center mb-4"><Calendar size={24} className="text-slate-400" /></div><h3 className="text-xl font-semibold text-white mb-2">No meals planned yet</h3><p className="text-slate-400 max-w-sm mx-auto">Start building your nutrition plan by adding your first meal.</p></div></div>)}
                </div>
              </Card>
            </div>
