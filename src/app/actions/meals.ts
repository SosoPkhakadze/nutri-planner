...
export async function addMeal(formData: FormData) {
  ...
  const date = formData.get('date') as string; // FIX: Get date from form data

  // FIX: Add validation for all fields
  if (!mealName || !mealTime || !date) {
    return { error: 'Meal name, time, and date are required.' };
  }

  // FIX: Use the correct date variable from the form
  const { error } = await supabase.from('meals').insert({ user_id: user.id, name: mealName, time: mealTime, date: date });
