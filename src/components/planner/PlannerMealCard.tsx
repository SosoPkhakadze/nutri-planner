...
interface PlannerMealCardProps {
  meal: {
    ...
    name: string;
    time: string;
    date: string; // Add date to the props
    meal_foods: any[];
  };
}
...
export default function PlannerMealCard({ meal }: PlannerMealCardProps) {
  ...
  return (
    <Link href={`/?date=${meal.date}`} className="block bg-slate-700 p-2 rounded-md text-sm cursor-pointer hover:bg-slate-600 transition-colors">
